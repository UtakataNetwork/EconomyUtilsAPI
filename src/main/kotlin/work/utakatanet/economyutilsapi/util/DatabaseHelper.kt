...
class DatabaseHelper {
    ...
    fun tableCreate() {
        try {
            ...
            if (!transactionLogTable.next()) {
                val ps = con.prepareStatement("CREATE TABLE `eco_transactionLog` (`id` INT AUTO_INCREMENT, `uuid` VARCHAR(100) NOT NULL DEFAULT '', `type` VARCHAR(50) NOT NULL DEFAULT '', `amount` DOUBLE NOT NULL DEFAULT 0, `reason` VARCHAR(1000) NOT NULL DEFAULT '', `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP, `updated_at` TIMESTAMP DEFAULT  CURRENT_TIMESTAMP, PRIMARY KEY (`id`));")
                ps.executeUpdate()
            }
        }catch (e: SQLException) {
     ...
     */
    fun addTransactionLog(uuid: UUID, type: TransactionType, amount: Double, reason: String): Boolean {
        try {
            val con = connection

            val psCreate = con.prepareStatement("INSERT INTO `eco_transactionLog` (`uuid`, `type`, `amount`, `reason`) VALUES (?, ?, ?, ?);")
            psCreate.setString(1, uuid.toString())
            psCreate.setString(2, type.name)
            psCreate.setDouble(3, amount)
            psCreate.setString(4, reason)
